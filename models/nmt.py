...
class NMT(nn.Module):
    ...
    def make_tgt_mask(self, tgt):
        ...
        tgt_pad_mask = (tgt != self.tgt_pad_idx).unsqueeze(1).unsqueeze(2) # [batch size, 1, 1, tgt len]

        tgt_len = tgt.size(1)
        tgt_sub_mask = torch.tril(torch.ones((tgt_len, tgt_len), device=self.device)).bool() # [tgt len, tgt len]

        tgt_mask = tgt_pad_mask & tgt_sub_mask # [batch size, 1, tgt len, tgt len]
        return tgt_mask
